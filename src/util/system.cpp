...
const char * const GEMONEY_CONF_FILENAME = "gemoney.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "gemoneyd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Gemoney
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Gemoney
    // Mac: ~/Library/Application Support/Gemoney
    // Unix: ~/.gemoney
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gemoney";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Gemoney";
#else
    // Unix
    return pathRet / ".gemoney";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", GEMONEY_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Gemoney Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Gemoney Core") == std::string::npos) {
