Gemoney Core version 0.11.0 is now available from:

  <https://gemoney.org/bin/gemoney-core-0.11.0/>
...
For example, add the following to `gemoney.conf`:
...
related to the blockchain in the gemoney system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Gemoney Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Gemoney Core will 
...
For an example script see [gemoney-submittx](https://github.com/laanwj/gemoney-submittx).
...
- #5334 `cf87536` libgemoneyconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new gemoney-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only GemoneyAmountField
- #5753 `0f44672` Add gemoney logo to about screen
...
- #5809 `46bfbe7` Add gemoney-cli man page
...
- #5849 `41259ca` contrib/init/gemoneyd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Gemoney-Qt.app to Gemoney-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for gemoney-tx)
