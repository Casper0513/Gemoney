Gemoney Core version 0.17.0 is now available from:

  <https://gemoneycore.org/bin/gemoney-core-0.17.0/>
...
installer (on Windows) or just copy over `/Applications/Gemoney-Qt` (on Mac)
or `gemoneyd`/`gemoney-qt` (on Linux).
...
Gemoney Core is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `gemoney.conf` file, not inside included files or from
  ...
  as gemoney.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the gemoney.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=gemoney
    test.uacomment=gemoney-testnet
...
be used in V0.17 by starting gemoneyd with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Gemoney Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/gemoney/bips/blob/master/bip-0174.mediawiki) is an interchange format for Gemoney transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://gemoneytalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed Gemoney transaction goes through the
following steps:
...
- An **Extractor** produces a valid Gemoney transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When gemoney is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If gemoney is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart gemoneyd with
...
- When running gemoneyd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If gemoneyd
...
- #11962 `0a01843` add seed.gemoney.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI gemoney:// (krab)
...
- #13528 `b877c39` Move GemoneyGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libgemoney-util to libgemoney-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for gemoney-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling gemoney-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use GemoneyTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to gemoney.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use GemoneyTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use GemoneyTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use GemoneyTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for GemoneyTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_gemoney` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.gemoney is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_gemoney` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in gemoneyd systemd service file (Flowdalic)
- #12652 `55f490a` gemoney-cli: Provide a better error message when gemoneyd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to gemoneyd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for gemoney address in translations (kaplanmaxe)
...
- #13603 `453ae5e` gemoney-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix gemoney-cli --version (Empact)
...
- #12877 `2b54155` Use gemoneyd in Tor documentation (knoxcard)
...
- #13346 `60f0358` update gemoney-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust gemoneycore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed gemoney-cli -help output for help2man (hebasto)
