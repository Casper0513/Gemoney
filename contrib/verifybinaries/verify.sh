...
###   gemoneycore.org and gemoney.org and compares them.
...
WORKINGDIR="/tmp/gemoney_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://gemoneycore.org"
HOST2="https://gemoney.org"
BASEDIR="/bin/"
VERSIONPREFIX="gemoney-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'gemoney-',
...
if [ $? -ne 0 ]; then
   echo "gemoney.org failed to provide signature file, but gemoneycore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "gemoney.org and gemoneycore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Gemoney Core binary release signing key installed?"
   fi
